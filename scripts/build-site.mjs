...
<a class="${page==='power-sprayer'?'active ':''}catalogue-pill" href="${url('power-sprayer')}">Power Sprayer</a>
<a class="${page==='htp-pump'?'active ':''}catalogue-pill" href="${url('htp-pump')}">HTP Pump</a>
<a class="${page==='battery-pump'?'active ':''}catalogue-pill" href="${url('battery-pump')}">Battery Pump</a>
</nav>`;
...
<a class="part-img-wrapper" href="${productUrl(item)}" aria-label="View details for ${escape(item.name)}"><img src="${escape(thumbUrl(item))}" alt="${escape(item.name)}" loading="lazy" width="600" height="450"></a>
<div class="part-info"><div class="product-badges"><span>${escape(item.catalogGroup)}</span><span>Direct enquiry</span></div><div class="item-num">${item.id}</div><h3><a class="part-title-link" href="${productUrl(item)}">${escape(item.name)}</a></h3><p class="part-group">Sprayer spare part</p><p class="part-price">Contact for price</p></div>
